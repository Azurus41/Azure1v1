...
class MyBot(Bot):
    ...
    def get_output(self, packet: GamePacket) -> ControllerState:
        ...
        V5 Specific
        '''
        def has_wheel_contact(car):
            return str(car.air_state) == "AirState.OnGround"
        def is_round_active():
            return str(packet.match_info.match_phase) != "MatchPhase.Paused"
        def is_kickoff_pause():
            return str(packet.match_info.match_phase) == "MatchPhase.Kickoff"
        def is_demolished(car):
            return car.demolished_timeout >= 0
        def double_jumped(car):
            return car.has_double_jumped == True or car.has_dodged == True
        def jumped(car):
            return car.has_jumped == True
        '''
        ...
        def get_preds():
            ...
            for i in range(1, 301):
                li.append(find_slice_at_time(self.ball_prediction, packet.match_info.seconds_elapsed + i / 60))
            return li
        ...
        def shot_variables_2(target):
            ...
            air_t = 0
            jump_vec = (292 * bool(not double_jumped(my_car)) + 146 * bool(not jumped(my_car)) + 146 / 24 * max(24 - abs(packet.match_info.frame_num - self.jump_tick), 0))
        ...
        def shoot_ball(target, li, other_cars, flick = False, double_jump = False, aerial = False, delay = False):
            ...
            # Drive
            will_aerial = aerial == True and my_car.boost / 100 * 3 >= target_t[3] + 0.1 * bool(has_wheel_contact(my_car)) and wait_for_fall[grounded_mode_index] >= 0
            if will_aerial:
            ...
            controls.boost = has_wheel_contact(my_car) == True and controls.throttle == 1 and car_velocity.length() < 2299 and should_boost == True and (wait_for_fall[grounded_mode_index] < 0 or travel_d[2] + travel_v[2] * (target_t[grounded_mode_index] - 1 / 60) < d_to_travel)
            ...
            if flip_ball.flat().dist(ball_location.flat() + ball_velocity.flat() * tick_t / 60) <= max((ball_velocity.flat().length() * tick_t / 60) * math.tan(math.pi / 20), travel_d[clamp(tick_t * 2 - round(tjt * 120), 0, 600)] / 25):
                ...
                if t >= 1.4 + test_jt:
                    ...
                    if has_wheel_contact(my_car) == True and abs(controls.steer) <= 0.1 and car_velocity.flat().length() >= 1000 + 1400 * bool(my_car.boost > 0):
            ...
            # Jumping
            if will_aerial:
                offset = bpos - (car_location + car_velocity * target_t[3] + Vec3(0, 0, -325) * target_t[3]**2) + up_direction * 292 * bool(not double_jumped(my_car))
                if has_wheel_contact(my_car) == False:
                    if my_car.boost / 100 * 3 >= target_t[3]:
                        point_toward(offset)
                        controls.boost = front_direction.ang_to(offset) < math.pi / 4
                        controls.throttle = 1
                        controls.jump = double_jumped(my_car) == False and (abs(packet.match_info.frame_num - self.jump_tick) < 24 or self.prev_jump == False)
                        target_roof_dir = (bpos - car_location)
                        controls.roll = sign(math.pi / 2 - get_angle(target_roof_dir, right_direction))
                        if jumped(my_car) == True and controls.jump == True and self.prev_jump == False:
                    ...
                    if steer_toward_target(my_car, bpos) <= 0.2:
                        controls.jump = True
                    point_toward(offset)
                self.renderer.draw_string_3d("Aerial", CarAnchor(self.index), 1, self.renderer.white)
            elif flick == True:
                if abs(self.just_flipped - packet.match_info.frame_num) > 78 and car_velocity.z < -10:
                    recover(bpos)
                jt, ja = new_jump_time(max(bpos.z - 130, 0), False)
                if (round((target_t[0] - jt) * 120) <= 0 and has_wheel_contact(my_car) == True) or (has_wheel_contact(my_car) == False and self.prev_jump == True):
                    controls.jump = True
                if has_wheel_contact(my_car) == False:
                ...
                self.renderer.draw_string_3d("Flick", CarAnchor(self.index), 1, self.renderer.white)
            elif double_jump == True:
                ...
                if (og_bpos.z <= 110 and bvel.z <= 100 and is_kickoff_pause() == False and abs(og_bpos.y - goal_location.y) - 92.75 >= (abs(car_velocity.y)**2 - 1000**2) / 1500):
                    self.should_chip = True
                elif abs(controls.steer) < 0.5:
                    self.should_chip = False
                # Jumping
                jt, ja = new_jump_time(max(bpos.z, 0), True)
                if has_wheel_contact(my_car) == True:
                ...
                elif my_car.latest_touch == None or round(my_car.latest_touch.game_seconds * 120) == self.prev_touch:
                    if abs(packet.match_info.frame_num - self.jump_tick) < 24:
                        controls.jump = True
                    else:
                        controls.jump = not self.prev_jump
                        if controls.jump == True and self.prev_jump == False and double_jumped(my_car) == False:
                            controls.pitch, controls.yaw, controls.roll = 0, 0, 0
                if double_jumped(my_car) == True and car_velocity.z > 0:
                ...
                self.renderer.draw_string_3d("Double Jump", CarAnchor(self.index), 1, self.renderer.white)
            else:
                # Adjust target for faster shots
                if bvel.flat().dist(car_velocity.flat()) > 2000:
                    bpos = Vec3(bpos.x, bpos.y, og_bpos.z + (bpos.z - og_bpos.z) * 2000 / bvel.flat().dist(car_velocity.flat()))
                ...
                if ((round((target_t[1] - jt) * 120) <= 0 and has_wheel_contact(my_car) == True and get_angle(front_direction.flat(), (bpos - car_location).flat()) <= math.pi / 2) or (has_wheel_contact(my_car) == False and self.prev_jump == True) or ((car_location + car_velocity / 20 + Vec3(0, 0, -325) / 400).dist(Vec3(ball_states[3].physics.location)) <= 150 and has_wheel_contact(my_car) == True)) and self.should_chip == False:
                    controls.jump = True
                if abs(packet.match_info.frame_num - self.jump_tick) >= 24 and abs(self.just_flipped - packet.match_info.frame_num) > 78:
                    point_toward((ball_location - car_location).flat())
                if has_wheel_contact(my_car) == False:
                    if (car_location + car_velocity / 60 + Vec3(0, 0, -325) / 3600).dist(Vec3(ball_states[1].physics.location)) <= 150 or my_car.latest_touch != None and round(my_car.latest_touch.game_seconds * 120) != self.prev_touch:
                        controls.jump = not self.prev_jump
                        if controls.jump == True:
                        ...
                        controls.jump = False
                self.renderer.draw_string_3d("Single Jump", CarAnchor(self.index), 1, self.renderer.white)
            if will_aerial == False and flick == False:
                if has_wheel_contact(my_car) == False and double_jumped(my_car) == False and my_car.latest_touch != None and round(my_car.latest_touch.game_seconds * 120) != self.prev_touch:
                    controls.jump = not self.prev_jump
                    if controls.jump == True:
                        controls.pitch = -math.floor(math.cos(front_direction.flat().ang_to(Vec3(ball_states[0].physics.location).flat() - car_location.flat())) + 0.5)
                        controls.yaw = math.floor(math.cos(right_direction.flat().ang_to(Vec3(ball_states[0].physics.location).flat() - car_location.flat())) + 0.5)
            ...
            # if controls.jump == True and self.prev_jump == False and double_jumped(my_car) == False and jumped(my_car) == True:
                # print(str(controls.pitch) + ";" + str(controls.yaw) + ";" + str(controls.roll) + ";" + str(controls.steer))
            controls.use_item = True
        
        def wall_shot(target, li, double_jump, aerial):
            target_t, target_f, wait_for_fall = li[0], li[1], li[2]
            used_index = double_jump
            will_aerial = aerial == True and my_car.boost / 100 * 3 >= target_t[3] + 0.1 * bool(has_wheel_contact(my_car)) and wait_for_fall[used_index] >= 0
            if car_location.z <= 50:
                used_f = ball_from_ground
                used_f2 = car_flat
            elif 4096 - abs(car_location.x) < 5120 - abs(car_location.y):
            ...
            controls.boost = has_wheel_contact(my_car) == True and controls.throttle == 1 and car_velocity.length() < 2299 and wait_for_fall[used_index] < 0
            controls.steer = steer_toward_target(my_car, target_pos)
            # Controls
            if will_aerial == True:
                offset = target_bpos - (car_location + car_velocity * target_t[3] + Vec3(0, 0, -325) * target_t[3]**2) + up_direction * 292 * bool(not double_jumped(my_car))
                if has_wheel_contact(my_car) == False:
                    if my_car.boost / 100 * 3 >= target_t[3]:
                        point_toward(offset)
                        controls.boost = front_direction.ang_to(offset) < math.pi / 4
                ...
                if used_index == 0:
                    if ((round((target_t[0] - jt) * 120) <= 0 and has_wheel_contact(my_car) == True and get_angle(front_direction, target_og_pos - car_location) <= math.pi / 2) or (has_wheel_contact(my_car) == False and self.prev_jump == True) or ((car_location + car_velocity / 20 + Vec3(0, 0, -325) / 400).dist(Vec3(ball_states[3].physics.location)) <= 150 and has_wheel_contact(my_car) == True)):
                        controls.jump = True
                    if has_wheel_contact(my_car) == True:
        ...
        def generic_demo(target):
            target_car = packet.players[target]
        ...
        def defense(goalie = False, fast = False):
            ...
            if my_car.boost < 100 and fast == False:
                best_score = math.inf
                for i in range(len(self.field_info.boost_pads)):
                    pad = self.field_info.boost_pads[i]
                    best_score = math.inf
                    if (abs(pad.location.x) > 3000 or abs(pad.location.y) > 4000) and Vec3(pad.location).dist(-goal_location) < car_location.dist(-goal_location) and sign(pad.location.x) == sign(car_location.x):
                        if packet.boost_pads[i].is_active == True or packet.boost_pads[i].timer <= (Vec3(pad.location) - car_location).length() / 2300:
        ...
        def general_strategy():
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900) and save_deadline < 0: # Side wall clears
                for i in other_cars[0]:
                    if is_demolished(packet.players[i]) == False:
                        cpos = Vec3(packet.players[i].physics.location)
                        furthest_score = min(cpos.dist(-goal_location), 900) + abs(cpos.y + goal_location.y)
                        front = car_dir(packet.players[i].physics.rotation.pitch, packet.players[i].physics.rotation.yaw)
                ...
                for i in other_cars[0]:
                    if is_demolished(packet.players[i]) == False:
                        ...
                        furthest_score = min(cpos.dist(-goal_location), 900) + abs(cpos.y + goal_location.y)
                        front = car_dir(packet.players[i].physics.rotation.pitch, packet.players[i].physics.rotation.yaw)
                        closest_score = cpos.dist(ball_location) + 5000 * bool((math.sin(packet.players[i].physics.rotation.yaw) * goal_location.y < 0 or (cpos.z <= 200 and double_jumped(packet.players[i]) == True and packet.players[i].physics.velocity.z < 10 and packet.players[i].physics.velocity.y * sign(goal_location.y) < 0)) or cpos.flat().dist(Vec3(sign(cpos.x, 1) * 900, -goal_location.y / 5120 * 5050, 0)) <= 100 or (cpos.y * sign(goal_location.y) <= -5050 and abs(cpos.x) <= 900 and front.y * sign(goal_location.y) < 0))
            ...
            if closest_to_ball[0] == self.index:
                ...
                if go_up_wall == True:
                    ...
                    to_double_jump = li[0][2] < li[0][1] and is_kickoff_pause() == False
                ...
                if self.defensive_stance == False and ball_location.y * sign(goal_location.y) > 0 and car_location.y * sign(goal_location.y) <= 50 and (sign(front_direction.y * goal_location.y) > 0 or car_location.y * sign(goal_location.y) < -4120):
                    ...
                    for i in other_cars[1]:
                        ...
                        if d < nearest_dist:
                            nearest_dist = d
                            nearest_opponent = packet.players[i]
        ...
        rot_vel = [(car_rotation.pitch - self.prev_rot[0]) * 120 / max(abs(packet.match_info.frame_num - self.prev_frame_nr), 1), yaw_conversion(self.prev_rot[1], car_rotation.yaw) * 120 / max(abs(packet.match_info.frame_num - self.prev_frame_nr), 1) * math.cos(car_rotation.pitch), roll_conversion(self.prev_rot[2], car_rotation.roll) * 120 / max(abs(packet.match_info.frame_num - self.prev_frame_nr), 1)]
        ...
        for i in range(len(packet.players)):
            if packet.players[i].name != "":
                other_cars[bool(packet.players[i].team != self.team)].append(i)
                team_sizes[bool(packet.players[i].team != self.team)] += 1
        ...
        # self.renderer.draw_string_3d(car_location, 1, 1, str(abs(self.just_flipped - packet.match_info.frame_num)), self.renderer.red())

        if controls.jump == True and self.prev_jump == False and has_wheel_contact(my_car) == True:
            self.jump_tick = packet.match_info.frame_num
            self.just_flipped = -10000
            if my_car.latest_touch != None:
                self.prev_touch = round(my_car.latest_touch.game_seconds * 120)
        if double_jumped(my_car) == False and jumped(my_car) == True and controls.jump == True and self.prev_jump == False and not (controls.yaw == controls.pitch == controls.roll == 0):
            self.just_flipped = packet.match_info.frame_num

        self.prev_jump = controls.jump
        self.prev_frame_nr = packet.match_info.frame_num
        self.prev_rot = [car_rotation.pitch, car_rotation.yaw, car_rotation.roll]
        if has_wheel_contact(my_car) == True:
            self.on_wall = car_location.z > 50

        # self.renderer.draw_string_3d(str(double_jumped(my_car)), CarAnchor(self.index), 1, self.renderer.white)
    ...
    def begin_front_flip(self, packet):
        ...
        self.active_sequence = Sequence([
            ControlStep(duration=0.08, controls=ControllerState(jump=True)),
            ControlStep(duration=0.02, controls=ControllerState(jump=False)),
            ControlStep(duration=0.65, controls=ControllerState(jump=True, pitch=-1)),
        ])
    ...
    def speed_flip(self, packet, dir):
        ...
        self.active_sequence = Sequence([
            ControlStep(duration=5/60, controls=ControllerState(steer = -dir, boost = True)),
            ControlStep(duration=1/60, controls=ControllerState(jump = True, boost = True)),
            ControlStep(duration=0.05, controls=ControllerState(jump = False, boost = True)),
            ControlStep(duration=1/60, controls=ControllerState(yaw = dir, jump = True, pitch = -1, boost = True)),
            ControlStep(duration=0.6, controls=ControllerState(pitch = 1, boost = True)),
            ControlStep(duration=0.2, controls=ControllerState(yaw = dir, roll = dir, boost = True)),
        ])
...
if __name__ == "__main__":
    ...
    MyBot("scumclass/noob_black_425").run()
